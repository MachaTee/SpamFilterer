...
# Load pandas
training_data = pd.read_csv('data\\spam_detection_training_data.csv').iloc[:2896, :]
test_data = pd.read_csv('data\\spam_detection_training_data.csv').iloc[2896:, :]
...
def chunk_data(string: str, length: int = WORD_SEQUENCE_LENGTH) -> List[str]:
    return list(string[x:length+x] for x in range(0, len(string), length))
...
def train(model: dict = model, training_data: pd.core.frame.DataFrame = training_data) -> dict:
    ...
    for data_index in range(len(normalised_data)):
        ...
        data_sequences = chunk_data(data_values.split())
        ...
        for sequence_index in range(len(data_sequences)):
            ...
            if model.get(sequence):
                ...
                if existing_sequence.chain.get(next_sequence):
                    existing_sequence.chain[next_sequence] += 1
                else:
                    existing_sequence.chain[next_sequence] = 1

            else:
                # Sequence does not exist, create new WordSequence object and append to model
                new_word_sequence = WordSequence(1, bool(label_data), {next_sequence: 1} if next_sequence else {}, data_sequences[sequence_index], len(data_sequences[sequence_index]))
...
def discriminate(model: dict = model, test_data: pd.core.frame.DataFrame = test_data, normalised_test_data: pd.core.frame.DataFrame = normalised_test_data, spam_threshold: int = SPAM_THRESHOLD) -> pd.core.frame.DataFrame:
    ...
    for data_index in range(len(normalised_test_data.values)):
        ...
        if spam_counter["not_spam"] / spam_counter["spam"] > spam_threshold:
            label_data.append(0)
        else:
            label_data.append(1)
